...
def numero_particulas(x, n):
    number_particles = qml.qchem.particle_number(n)
    number_particles = np.array( qml.matrix(number_particles, wire_order=[i for i in range(n)]) )

    state = x
    state_dag = x.T.conj()
    val = state.dot(number_particles).dot(state_dag)
    return np.round(np.real(val),3)
...
def params(n, t, u, p, lr, flag):
    ...
    if flag == 0:
        ...
        doubles = len(doubles)
        minimizate_params = {
            "maxiter": 2**( ansatz_params["qubits"]+1 ),
            "tol": 1e-6,
