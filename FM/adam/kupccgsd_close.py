...
for n in N:
    for u in U:
        ...
        aux.to_csv("datos/adam/2KEnergyClose"+str(n)+"u"+str(u)+".csv")

        aux = pd.DataFrame(theta)
        aux.to_csv("datos/adam/2KThetaClose"+str(n)+"u"+str(u)+".csv")

        aux = [get_state_kUpCCGSD(angle, p3) for angle in theta]
        aux = pd.DataFrame(aux)
        aux.to_csv("datos/adam/2KStateClose"+str(n)+"u"+str(u)+".csv")
