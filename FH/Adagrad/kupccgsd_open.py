...
for n in N:
    for t in T:
        for u in U:
            ...
            aux.to_csv("FH/datos/adagrad"+str(lr)+"-KEnergyOpen"+str(n)+"t"+str(t)+"u"+str(u)+".csv")
            aux = pd.DataFrame(theta)
            aux.to_csv("FH/datos/adagrad"+str(lr)+"-KThetaOpen"+str(n)+"t"+str(t)+"u"+str(u)+".csv")
            aux = [get_state_kUpCCGSD(angle, p3) for angle in theta]
            aux = pd.DataFrame(aux)
            aux.to_csv("FH/datos/adagrad"+str(lr)+"-KStateOpen"+str(n)+"t"+str(t)+"u"+str(u)+".csv")