...
for n in N:
    for t in T:
        for u in U:
            ...
            aux.to_csv("FH/datos/adam"+str(lr)+"-EnergyClose"+str(n)+"t"+str(t)+"u"+str(u)+".csv")

            aux = pd.DataFrame(theta)
            aux.to_csv("FH/datos/adam"+str(lr)+"-ThetaClose"+str(n)+"t"+str(t)+"u"+str(u)+".csv")

            aux = [get_state_UCCSD(angle, p3) for angle in theta]
            aux = pd.DataFrame(aux)
            aux.to_csv("FH/datos/adam"+str(lr)+"-StateClose"+str(n)+"t"+str(t)+"u"+str(u)+".csv")