...
for d in distance:
        ...
        aux.to_csv("Mol/datos/adam"+str(lr)+"-K3HEnergy"+str(d)+".csv")
        aux = pd.DataFrame(theta)
        aux.to_csv("Mol/datos/adam"+str(lr)+"-K3HTheta"+str(d)+".csv")
        aux = [get_state_kUpCCGSD(angle, p2) for angle in theta]
        aux = pd.DataFrame(aux)
        aux.to_csv("Mol/datos/adam"+str(lr)+"-K3HStates"+str(d)+".csv")