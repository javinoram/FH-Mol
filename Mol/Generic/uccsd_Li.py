...
for d in distance:
        ...
        aux.to_csv("Mol/datos/generic"+str(lr)+"-LiEnergy"+str(d)+".csv")

        aux = pd.DataFrame(theta)
        aux.to_csv("Mol/datos/generic"+str(lr)+"-LiTheta"+str(d)+".csv")

        aux = [get_state_UCCSD(angle, p2) for angle in theta]
        aux = pd.DataFrame(aux)
        aux.to_csv("Mol/datos/generic"+str(lr)+"-LiStates"+str(d)+".csv")