...
for d in distance:
        ...
        aux.to_csv("datos/exacto/3HEnergy"+str(d)+".csv")

        aux = [states[:,i] for i in range(len(energy))]
        aux = pd.DataFrame(aux)
        aux.to_csv("datos/exacto/3HState"+str(d)+".csv")
...
for d in distance:
        ...
        aux = pd.DataFrame(energy)
        aux.to_csv("datos/exacto/LiEnergy"+str(d)+".csv")

        aux = [states[:,i] for i in range(len(energy))]
        aux = pd.DataFrame(aux)
        aux.to_csv("datos/exacto/LiState"+str(d)+".csv")