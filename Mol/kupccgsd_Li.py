...
for d in distance:
        print(d)
        symbols, coor, p1, p2, p3 = paramsLi(d, 'sto-3g', 0.2, 1)
        energy, theta = complete_flow_KUPCCGSD(symbols, coor, p1, p2, p3)

        aux = pd.DataFrame(energy)
        aux.to_csv("datos/Mol/KLiEnergy"+str(d)+".csv")

        aux = pd.DataFrame(theta)
        aux.to_csv("datos/Mol/KLiTheta"+str(d)+".csv")

        aux = [get_state_kUpCCGSD(angle, p2) for angle in theta]
        aux = pd.DataFrame(aux)
        aux.to_csv("datos/Mol/KLiStates"+str(d)+".csv")